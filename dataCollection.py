...
for route in ri.routeLst:
    ...
    if os.stat(f'{route}.csv').st_size == 0:
        header = ['Time', 'Vehicle_id', 'Speed', 'Passenger_load', 'Next_stop', 'Latitude', 'Longitude', 'Heading', 'Temperature', 'Windspeed', 'Precipitation', 'Humidity', 'Visibility', 'Stop_distance']
        with open(f'{route}.csv', 'w', encoding='UTF8') as file:
            writer = csv.writer(file)
